					...
					<div style="width:50%;border:1px solid black;position:relative;">
						<table class="day-book-table">
							...
							<tbody>
								...
								<template v-if="material_sales.length > 0">
									<tr>
										<td class="sub-heading">Material Sales</td>
										<td class="sub-value">{{ totalMaterialSales }}</td>
									</tr>
									<tr v-for="sale in material_sales">
										<td>{{ sale.Customer_Name }}</td>
										<td>{{ sale.totalAmount | decimal }}</td>
									</tr>
								</template>
...
<script>
	new Vue({
		...
		computed: {
			...
			totalMaterialSales() {
				return this.material_sales.reduce((prev, curr) => {
					return prev + parseFloat(curr.totalAmount)
				}, 0).toFixed(2);
			},
			totalAssetPurchases() {
		...
		methods: {
			...
			getMaterialSales(){
				...
				axios.post('/get_material_cost', filter)
				.then(res => { 
					let material_sales = res.data.materialSales.filter(sale => sale.paid > 0);
					material_sales = _.groupBy(material_sales, 'customer_id');
					material_sales = _.toArray(material_sales);
					material_sales = material_sales.map(sale => {
						sale[0].totalAmount = sale.reduce((p, c) => { return p + parseFloat(c.paid) }, 0);
						return sale[0];
					})
