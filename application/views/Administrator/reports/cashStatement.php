		...
		<div class="row" style="padding-top:10px;">
			<div class="col-md-6">
				...
				<table class="table table-bordered table-condensed">
					...
					<tbody style="display:none;" v-bind:style="{display: materialSales.length > 0 ? '' : 'none'}">
						<tr v-for="payment in materialSales">
							<td>{{ payment.sales_id }}</td>
							<td>{{ payment.invoice_no }}</td>
							<td style="text-align:right;">{{ payment.paid | decimal }}</td>
						</tr>
					</tbody>
					<tfoot>
						<tr style="font-weight:bold;">
							<td colspan="2" style="text-align:right">Total</td>
							<td style="text-align:right;">
								<span v-if="materialSales.length == 0">0.00</span>
								<span style="display:none;" v-bind:style="{display: materialSales.length > 0 ? '' : 'none'}">{{ totalMaterialSales | decimal }}</span>
							</td>
						</tr>
					</tfoot>
				</table>
	...
	new Vue({
		...
		methods: {
			...
			getMaterialSales(){
				...
				.then(res => { 
					this.materialSales = res.data.materialSales;
				})
			},
			getLoanPayments() {
				let filter = {
