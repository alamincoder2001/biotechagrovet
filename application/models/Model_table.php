...
class Model_Table extends CI_Model
{
    ...
    public function generateMaterialSalesCode()
    {
        $code = "MS-0001";

        $lastRow = $this->db->query("select * from tbl_material_sales order by sales_id desc limit 1");
        if ($lastRow->num_rows() != 0) {
            $newCode = $lastRow->row()->sales_id + 1;
            $zeros = array('0', '00', '000');
            $code = 'MP-' . (strlen($newCode) > count($zeros) ? $newCode : $zeros[count($zeros) - strlen($newCode)] . $newCode);
        }
    ...
    public function getTransactionSummary($date = null)
    {
        $transactionSummary = $this->db->query("
            ...
            (
                select ifnull(sum(ms.paid), 0) from tbl_material_sales ms
                where ms.branch_id = " . $this->session->userdata('BRANCHid') . "
                and ms.status = 'a'
                " . ($date == null ? "" : " and ms.sales_date < '$date'") . "
            ) as received_material_sale,
            ...
            /* total */
            (
                select received_sales + received_customer + received_supplier + received_cash + bank_withdraw + loan_received + loan_initial_balance + invest_received + sale_asset + received_material_sale
            ) as total_in,
            (
    ...
    public function customerDue($clauses = "", $date = null)
    {
        ...
        $dueResult = $this->db->query("
            ...
            (select ifnull(sum(ms.total), 0) from tbl_material_sales ms
                where ms.customer_id = c.Customer_SlNo
                and ms.status = 'a'
                " . ($date == null ? "" : " and ms.sales_date < '$date'") . "
            ) as material_bill,
            ...
            (select invoicePaid + cashReceived + paid_for_material) as paidAmount,

            (select (billAmount + paidOutAmount + material_bill) - (paidAmount + returnedAmount)) as dueAmount
            
            from tbl_customer c
            left join tbl_unit_area ua on ua.unit_area_id = c.unit_area_id
            left join tbl_territories tr on tr.territory_id = c.territory_id
